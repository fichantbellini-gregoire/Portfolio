...
//Liste des coup
const char HAUT_SANS_CAISSE = 'z';
const char BAS_SANS_CAISSE = 's';
const char DROITE_SANS_CAISSE = 'd';
const char GAUCHE_SANS_CAISSE = 'q';
const char HAUT_AVEC_CAISSE = 'Z';
const char BAS_AVEC_CAISSE = 'S';
const char DROITE_AVEC_CAISSE = 'D';
const char GAUCHE_AVEC_CAISSE = 'Q';


const int ZERO = 0;
const int VALEUR_INTERDITE = -1;
...
void dectection_mouv_sans_caisse(t_tabDeplacement t,int *deplacement);
void dectection_mouv_avec_caisse(t_tabDeplacement t,int *deplacement);


void annuler_deplacement(t_plateau plateau, int *x, int *y, t_tabCible cibles, int *deplacementCorrigé, t_tabDeplacement t_deplacement,t_jeu *jeux,int *nbrDeplacement);
void fusionner_plateaux(t_plateau plateau, t_tabCible cibles);
bool transDep2Mouv(char *dep);
...
void mise_a_jour_caisse(t_jeu *jeux,char touche);
void mise_a_jour_joueur(t_jeu *jeux,int *joueurX ,int *joueurY);
void modif_caisse(char touche,t_cordonnees caisse);
void mise_a_jour_caisse_sans_mouvement(t_jeu *jeux);
bool verif_caisse(t_caisse_instant actuel,t_caisse_instant avant,t_caisse_instant non_bouge);
int verif_jeu(t_jeu *jeux,t_caisse_instant caisse);
void optimisation(t_jeu *jeux,int indice_retour,t_tabDeplacement tableauDeplacement,int *nbdeplacement,int *deplacementCorrigé);
int joueur_sur_caisse(t_jeu *jeux,int limite);
int nombre_caisse(t_caisse_instant caisse);
void trouver_caisse(t_caisse_instant caisse,t_plateau plateau);
void modif_joueur(char touche,t_cordonnees joueur);
...
int main(){
    ...
    int deplacementCorrigé=0; // le nombre de déplacement de la version corrigé
    char touche;
    ...
    bool mouv_caisse;
    int indice_retour;
    t_caisse_instant caisse_immobile;
    bool deplace_caisse;
    ...
    //recherche des variables au lancement
    ...
    trouver_caisse(caisse_immobile,plateauJeu);
    ...
    while ((!gagner(plateauJeu,ciblesNiveau)) && nbrDeplacement < deplacementTotal+1){//nbrDeplacement nous servira d'indice
        ...
        touche =tabMouvement[nbrDeplacement];
        deplace_caisse=transDep2Mouv(&touche);




        if (touche == ANNULER_COUP){
            annuler_deplacement(plateauJeu,&joueurX,&joueurY,ciblesNiveau,&deplacementCorrigé,tabMouvement,&jeux,&nbrDeplacement);
            ...
            if(jeux.joueur[jeux.etat_actuel][0]!= joueurX || jeux.joueur[jeux.etat_actuel][1]!= joueurY){//si le joueur c'est deplacer
                    if(mouv_caisse==deplace_caisse){ //si le joueur ne pousse pas le vide ou n'avance pas dans une caisse
                        mise_a_jour_joueur(&jeux,&joueurX,&joueurY);

                        if(mouv_caisse){
                            mise_a_jour_caisse(&jeux,touche);
                        }
                        else{
                            mise_a_jour_caisse_sans_mouvement(&jeux);
                        }

                        indice_retour = verif_jeu(&jeux,caisse_immobile);
                        if(indice_retour != VALEUR_INTERDITE){
                            optimisation(&jeux,indice_retour,tableauDeplacement,&nbrDeplacement,&deplacementCorrigé);
                        }
                    }
                    else{//si le joueur a fait un mouvement impossible
                        annuler_deplacement(plateauJeu,&joueurX,&joueurY,ciblesNiveau,&nbrDeplacement,tabMouvement,&jeux,&nbrDeplacement);
                    }
            }
    ...
    if (gagner(plateauJeu,ciblesNiveau)){
        ...
        if (depEnTrop == 0){
            ...
            printf("Elle contient initialement %d de caractères.\n",nbrDeplacement);
            printf("Après optimisation, elle contient %d de caractères.\n", deplacementCorrigé);
            enregistrerDeplacements(tableauDeplacement,deplacementCorrigé,fichierChoisiDep);
        }
    }
    else{
...
t_tabCible cibles, int *nbDeplacements, t_tabDeplacement tDeplacement){
    ...
    switch (touche){
        case 'h': // représente la touche pour aller vers le haut.
                mouv = deplacer_haut(plateau, x, y, cibles, nbDeplacements, tDeplacement);
            break;

        case 'b': // représente la touche pour aller vers le bas.
                mouv = deplacer_bas(plateau, x, y, cibles, nbDeplacements, tDeplacement);
            break;

        case 'd': // représente la touche pour aller vers la droite.
                mouv = deplacer_droite(plateau, x, y, cibles, nbDeplacements, tDeplacement);
            break;

        case 'g': // représente la touche pour aller vers la gauche.
                mouv = deplacer_gauche(plateau, x, y, cibles, nbDeplacements, tDeplacement);
            break;
...
/**
*
* @brief Procédure permettant d'obtenir les cordonnées du joueur sur le plateau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
* @param plateau de type t_plateau, Entrée/Sortie : le plateau de jeu. 
...
t_tabCible cibles, int *nbDeplacements, t_tabDeplacement tabloDeplacement){
    ...
    if ((*x > 0) && (plateau[*x - 1][*y] == CASE)){ 
        ...
        *x = (*x) - 1;
        tabloDeplacement[*nbDeplacements] = HAUT_SANS_CAISSE;
        (*nbDeplacements)++;
    ...
    (plateau[*x - 1][*y] == BOITE_SUR_CIBLE))){
        if ((plateau[*x - 2][*y] == CASE) || (plateau[*x - 2][*y] == CIBLE)){
            ...
            plateau[*x - 2][*y] = BOITE;
            *x = (*x) - 1;
            tabloDeplacement[*nbDeplacements] = HAUT_AVEC_CAISSE;
            mouv = true;
            (*nbDeplacements)++;
...
t_tabCible cibles, int *nbDeplacements, t_tabDeplacement tabloDeplacement){
    ...
    if ((*y > 0) && (plateau[*x][*y - 1] == CASE)){
        ...
        plateau[*x][*y] = cibles[*x][*y];
        *y = (*y) - 1;
        tabloDeplacement[*nbDeplacements] = GAUCHE_SANS_CAISSE;
        (*nbDeplacements)++;
    ...
    (plateau[*x][*y - 1] == BOITE_SUR_CIBLE))){
        if ((plateau[*x][*y - 2] == CASE) || (plateau[*x][*y - 2] == CIBLE)){
            ...
            plateau[*x][*y - 2] = BOITE;
            *y = (*y) - 1;
            tabloDeplacement[*nbDeplacements] = GAUCHE_AVEC_CAISSE;
            mouv = true;
            (*nbDeplacements)++;
...
t_tabCible cibles, int *nbDeplacements, t_tabDeplacement tabloDeplacement){
    ...
    if ((*x < TAILLE) && (plateau[*x + 1][*y] == CASE)){
        ...
        plateau[*x][*y] = cibles[*x][*y];
        *x = (*x) + 1;
        tabloDeplacement[*nbDeplacements] = BAS_SANS_CAISSE;
        (*nbDeplacements)++;
    ...
    (plateau[*x + 1][*y] == BOITE_SUR_CIBLE))){
        if ((plateau[*x + 2][*y] == CASE) || (plateau[*x + 2][*y] == CIBLE)){
            ...
            plateau[*x + 2][*y] = BOITE;
            *x = (*x) + 1;
            tabloDeplacement[*nbDeplacements] = BAS_AVEC_CAISSE;
            mouv  = true;
            (*nbDeplacements)++;
...
t_tabCible cibles, int *nbDeplacements, t_tabDeplacement tabloDeplacement){
    ...
    if ((*y < TAILLE - 1) && (plateau[*x][*y + 1] == CASE)){
        ...
        plateau[*x][*y] = cibles[*x][*y];
        *y = (*y) + 1;
        tabloDeplacement[*nbDeplacements] = DROITE_SANS_CAISSE;
        (*nbDeplacements)++;
    ...
    (plateau[*x][*y + 1] == BOITE_SUR_CIBLE))){
        if ((plateau[*x][*y + 2] == CASE) || (plateau[*x][*y + 2] == CIBLE)){
            ...
            plateau[*x][*y + 2] = BOITE;
            *y = (*y) + 1;
            tabloDeplacement[*nbDeplacements] = DROITE_AVEC_CAISSE;
            mouv = true;
            (*nbDeplacements)++;
...
* où sont mémorisés les déplacements du joueur.
*/
void annuler_deplacement(t_plateau plateau, int *x, int *y, t_tabCible cibles, int *deplacementCorrigé, 
    t_tabDeplacement t_deplacement,t_jeu *jeux,int *nbrDeplacement){

    if (t_deplacement[*deplacementCorrigé - 1] == HAUT_SANS_CAISSE){
        plateau[*x + 1][*y] = JOUEUR;
        plateau[*x][*y] = cibles[*x][*y];
        *x = (*x) + 1;

    } else if (t_deplacement[*deplacementCorrigé - 1] == HAUT_AVEC_CAISSE){
        plateau[*x + 1][*y] = JOUEUR;
        plateau[*x][*y] = BOITE;
        plateau[*x - 1][*y] = cibles[*x - 1][*y];
        *x = (*x) + 1;

    } else if (t_deplacement[*deplacementCorrigé - 1] == BAS_SANS_CAISSE){
        plateau[*x - 1][*y] = JOUEUR;
        plateau[*x][*y] = cibles[*x][*y];
        *x = (*x) - 1;

    } else if (t_deplacement[*deplacementCorrigé - 1] == BAS_AVEC_CAISSE){
        plateau[*x - 1][*y] = JOUEUR;
        plateau[*x][*y] = BOITE;
        plateau[*x + 1][*y] = cibles[*x + 1][*y];
        *x = (*x) - 1;

    } else if (t_deplacement[*deplacementCorrigé - 1] == GAUCHE_SANS_CAISSE){
        plateau[*x][*y - 1] = JOUEUR;
        plateau[*x][*y] = cibles[*x][*y];
        *y = (*y) - 1;

    } else if (t_deplacement[*deplacementCorrigé - 1] == GAUCHE_AVEC_CAISSE){
        plateau[*x][*y - 1] = JOUEUR;
        plateau[*x][*y] = BOITE;
        plateau[*x][*y + 1] = cibles[*x][*y + 1];
        *y = (*y) - 1;

    } else if (t_deplacement[*deplacementCorrigé - 1] == DROITE_SANS_CAISSE){
        plateau[*x][*y + 1] = JOUEUR;
        plateau[*x][*y] = cibles[*x][*y];
        *y = (*y) + 1;
    ...
    optimisation(jeux,(*nbrDeplacement-1),t_deplacement,&(*nbrDeplacement),&(*deplacementCorrigé));
...
/*Le principe de l'optimisation c'est qu'on enregistre a chaque etape ou sont le joueur et les caisses et si il y a un moment ou deux etapes 
sont identique on efface ce qu'il y a eu entre les deux*/


void init_jeu(t_jeu *jeux,t_plateau plateauJeu){//init la structure jeu au debut de parti
    jeux -> etat_actuel = ZERO;
    init_joueur(jeux->joueur,jeux->etat_actuel);
...
void init_joueur(t_joueur joueur,int debut){//permet d'initiliser tout les etat d'un joueur a partir de l'etat "debut"
    int i;
...
void init_caisse(t_caisse caisse_bouge,int debut){//permet d'initiliser tout les etat des caisses a partir de l'etat "debut"
    int i;
    int j;
    for(i = debut ; i < TAILLE_MAX_DEPLACEMENT;i++){//un boucle pour les etat
        for(j = ZERO ; j < NB_CAISSE_MAX ;j++){//une boucle pour les caisses
...
void trouver_caisse(t_caisse_instant caisse,t_plateau plateau){//permet de trouver toute les caisses d'un plateau et rempli de caisse vide si il en trouve plus
    int i = ZERO;
    int j;
    int col;
    int lig;
    for (lig = ZERO ; lig < TAILLE ; lig++){//double boucle pour le tableau
    ...
    for(j = i ; j < NB_CAISSE_MAX ; j++){//on remplis ce qui est vide
        caisse[j][0] = ZERO;
        caisse[j][1] = ZERO;
    }
}
void mise_a_jour_joueur(t_jeu *jeux,int *joueurX ,int *joueurY){
    if (jeux->etat_actuel+1 <= TAILLE_MAX_DEPLACEMENT){//on evite l'erreur de segmentation
        ...
        jeux->etat_actuel++;//on incremente l'etat de jeu

        //on prend la derniere place connu du joueur donc celle après mouvement
        jeux->joueur[jeux->etat_actuel][0] = *joueurX;
        jeux->joueur[jeux->etat_actuel][1] = *joueurY;
...
void modif_caisse(char touche,t_cordonnees caisse){//on modifie les cordonnée de la caisse passé en parametre selon la touche
    switch(touche){
        case('z'):  
                    caisse[0]--;
                    break;
...
void mise_a_jour_caisse(t_jeu *jeux,char touche){ //nouvelle etat avec une caisse qui c'est deplacer
    int i = ZERO;
    int ind_caisse;
    bool plus_caisse=false;
    if(jeux->etat_actuel > ZERO){
        while(i < TAILLE_MAX_DEPLACEMENT && !plus_caisse){//on reprend toute les positions connus des caisse
            jeux->caisse_bouge[jeux->etat_actuel][i][0] = jeux->caisse_bouge[jeux->etat_actuel-1][i][0];
            jeux->caisse_bouge[jeux->etat_actuel][i][1] = jeux->caisse_bouge[jeux->etat_actuel-1][i][1];
            if (jeux->caisse_bouge[jeux->etat_actuel][i+1][0]==ZERO){
                plus_caisse=true;
            }
            i++;
        }
        ind_caisse=joueur_sur_caisse(jeux,i);//on trouve l'indice de la caisse qui c'est déplacer

        if(ind_caisse==VALEUR_INTERDITE){//si la caisse n'existe pas on la crée
            ind_caisse = i;
            jeux->caisse_bouge[jeux->etat_actuel][ind_caisse][0] = jeux->joueur[jeux->etat_actuel][0];
            jeux->caisse_bouge[jeux->etat_actuel][ind_caisse][1] = jeux->joueur[jeux->etat_actuel][1]; 
        }

        modif_caisse(touche,jeux->caisse_bouge[jeux->etat_actuel][ind_caisse]);
    }   
}

int joueur_sur_caisse(t_jeu *jeux,int limite){ //retourne l'indice de sur la caisse qui c'est faite pousser par le joueur (-1 si la caisse n'existe pas)
    int i=ZERO;
    bool trouve = false;
    while(i < limite && !trouve){//recherche séquentielle de quand la joueur possede les meme coordoonés qu'une caisse
        if(jeux->joueur[jeux->etat_actuel][0]==jeux->caisse_bouge[jeux->etat_actuel][i][0] && jeux->joueur[jeux->etat_actuel][1]==jeux->caisse_bouge[jeux->etat_actuel][i][1]){
            trouve = true;
        }
        else{
            i++;
    ...
    if(!trouve){//si aucune caisse n'a ete trouvé
        i = VALEUR_INTERDITE;
    }
    return i;
}

void mise_a_jour_caisse_sans_mouvement(t_jeu *jeux){ //un nouvel etat lorsque aucune caisse ne c'est déplacer
    int i = ZERO;
    bool plus_caisse=false;
    if(jeux->etat_actuel > ZERO){
        while(i < TAILLE_MAX_DEPLACEMENT && !plus_caisse){//on recopie le placement du dernier etat
            jeux->caisse_bouge[jeux->etat_actuel][i][0] = jeux->caisse_bouge[jeux->etat_actuel-1][i][0];
            jeux->caisse_bouge[jeux->etat_actuel][i][1] = jeux->caisse_bouge[jeux->etat_actuel-1][i][1];
            if (jeux->caisse_bouge[jeux->etat_actuel][i+1][0]==ZERO){
                plus_caisse=true;
...
int verif_jeu(t_jeu *jeux,t_caisse_instant caisse){//retourne l'indice du plateau avec un etat identique (-1 si pas trouver)
    int i = ZERO;
    bool trouver = false;
    while (!trouver && i < jeux->etat_actuel){
        if(jeux->joueur[jeux->etat_actuel][0] == jeux->joueur[i][0] && jeux->joueur[jeux->etat_actuel][1] == jeux->joueur[i][1]){
            //on fait les calculs des caisses uniquement si le joueur est au meme emplacement
            trouver = verif_caisse(jeux->caisse_bouge[jeux->etat_actuel],jeux->caisse_bouge[i],caisse);
            //verif caisse est une fonction booléen donc on s'en sert pour  la variabel trouver
        }
        else{
            i++;
        }
    }
    if(!trouver){//si on n'a pas trouver d'etat identique
        i = VALEUR_INTERDITE;
    }
    return i;
...
int nombre_caisse(t_caisse_instant caisse){//retourne le nombre de caisse qui ont été deplacé
    int i = ZERO;
    while (caisse[i][0]!=ZERO && i < NB_CAISSE_MAX){
        i++;
    }
    return i;
}

bool verif_caisse(t_caisse_instant actuel,t_caisse_instant avant,t_caisse_instant non_bouge){//compare deux etat de caisse est retourne true si elle sont identique
    int nb_actuel = nombre_caisse(actuel);
    int nb_avant = nombre_caisse(avant);
    int nb_non_bouge = nombre_caisse(non_bouge);
    int cpt = ZERO;
    int autreCpt;
    bool est_le_meme = true;//de base les deux sont considere identique
    bool trouver;

    while (cpt < nb_actuel && est_le_meme){//on parcourt toute les caisses de l'etat actuel tant qu'on a pas reussi a prouver que les etat n'etait pas identique
        trouver = false;
        autreCpt = ZERO;
        while(!trouver && autreCpt < nb_avant){//d'abord on fait parmi les caisse qui ont ete deplace au moins une fois
            if(actuel[cpt][0] == avant[autreCpt][0] && actuel[cpt][1] == avant[autreCpt][1]){
                trouver = true;
            }
            autreCpt++;
        }

        if(!trouver){//si on a pas trouver on regarde les caisse qui n'ont jamais bougé (pour l'inversment de caisse)
            autreCpt = ZERO;
            while(!trouver && autreCpt < nb_non_bouge){
                if(actuel[cpt][0] == non_bouge[autreCpt][0] && actuel[cpt][1] == non_bouge[autreCpt][1]){
                    trouver = true;
                }
                autreCpt++;
            }
        ...
        if(!trouver){//si on a pas trouver de caisse identique 
            est_le_meme = false;//alors les deux etat ne sont pas identique
        }
...
void optimisation(t_jeu *jeux,int indice_retour,t_tabDeplacement tableauDeplacement,int *nbdeplacement,int *deplacementCorrigé){
//retire de la structure et de l'historique tout les etat inutiles entre l'etat actuel et un etat antecedant


int i = *nbdeplacement;
    init_joueur(jeux->joueur,indice_retour+1);//on reinitilise les etat de joueur en excluant l'etat qu'on souhaite retourné
    init_caisse(jeux->caisse_bouge,indice_retour+1);//on reinitilise les etat des caisses en excluant l'etat qu'on souhaite retourné

    while(i > indice_retour){//on retire de l'historique tout les etapes inutiles
        tableauDeplacement[i] = VIDE;
        i--;
    }

    *deplacementCorrigé = *deplacementCorrigé-i;
}